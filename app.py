...
def professor_dashboard():
    ...
    ''', (current_user.id,))
    total_alunos = cur.fetchone()[0]  # fetchone() retorna uma tupla
...
def relatorio_turma(turma_id):
    ...
    cur.execute('''
        ...
        ORDER BY u.first_name, u.last_name
    ''', (turma[2], current_user.id, turma_id))  # turma[2] = serie
    alunos_progresso = cur.fetchall()
    
    # Estatísticas da turma
    total_alunos = len(alunos_progresso)
    if total_alunos > 0:
        # Os dados vêm como tuplas: (id, first_name, last_name, username, total_aulas, aulas_concluidas, total_pontos, media_pontos)
        media_turma = sum(aluno[7] or 0 for aluno in alunos_progresso) / total_alunos
        total_aulas_turma = alunos_progresso[0][4] if alunos_progresso else 0
    else:
        media_turma = 0
        total_aulas_turma = 0
...
def relatorio_aluno(aluno_id):
    ...
    ''', (aluno_id, aluno_info[5], current_user.id))  # aluno_info[5] = turma_serie
    progresso_aulas = cur.fetchall()
    
    # Estatísticas do aluno
    total_aulas = len(progresso_aulas)
    aulas_concluidas = len([a for a in progresso_aulas if a[4] == 'concluido'])  # a[4] = status
    total_pontos = sum([a[5] or 0 for a in progresso_aulas])  # a[5] = pontuacao
    tempo_total = sum([a[6] or 0 for a in progresso_aulas])  # a[6] = tempo_assistido
...
def admin_relatorio_usuarios():
    ...
    try:
        ...
        flash(f'❌ Erro ao carregar relatório 0002: {str(e)}', 'error')
...
def admin_relatorio_turmas():
    ...
    try:
        ...
        flash(f'❌ Erro ao carregar relatório 0001: {str(e)}', 'error')
        return redirect(url_for('admin_relatorios'))
...
def student_gamificacao():
    ...
    try:
        ...
        # Converter sqlite3.Row para lista se necessário
        if nivel_info and hasattr(nivel_info, 'keys'):
            nivel_info = list(nivel_info)
        
        # Debug: verificar o que está sendo retornado
        print(f"DEBUG - nivel_info type: {type(nivel_info)}")
        print(f"DEBUG - nivel_info value: {nivel_info}")
        if nivel_info:
            print(f"DEBUG - nivel_info is iterable: {hasattr(nivel_info, '__iter__')}")
            if hasattr(nivel_info, '__len__'):
                print(f"DEBUG - nivel_info length: {len(nivel_info)}")
        ...
        try:
            ...
            print(f"DEBUG - metas_semanais type: {type(metas_semanais)}")
            print(f"DEBUG - metas_semanais value: {metas_semanais}")
        except Exception as e:
            print(f"DEBUG - Erro ao buscar metas: {e}")
            metas_semanais = []
        ...
        import traceback
        print(f"ERRO COMPLETO na gamificação: {str(e)}")
...
def forum():
    ...
    try:
        ...
        # Buscar tópicos com informações dos autores e aulas
        cur.execute('''
            SELECT t.id, t.titulo, t.conteudo, t.tipo, t.status, t.data_criacao, t.visualizacoes,
                   u.username as autor_nome, u.user_type as autor_role,
                   a.titulo as aula_titulo,
                   COUNT(r.id) as num_respostas,
                   GROUP_CONCAT(tag.nome) as tags
            FROM forum_topicos t
            JOIN users u ON t.autor_id = u.id
            JOIN aulas a ON t.aula_id = a.id
            LEFT JOIN forum_respostas r ON t.id = r.topico_id AND r.ativo = 1
            LEFT JOIN forum_topicos_tags tt ON t.id = tt.topico_id
        ...
        # Buscar aulas disponíveis para filtro
        cur.execute('SELECT id, titulo FROM aulas ORDER BY titulo')
        aulas = cur.fetchall()
        
        # Buscar tags disponíveis
        cur.execute('SELECT id, nome, cor FROM forum_tags ORDER BY nome')
        tags = cur.fetchall()
        
        cur.close()
        
        return render_template('forum.html', topicos=topicos, aulas=aulas, tags=tags)
        
    except Exception as e:
        flash(f'Erro ao carregar fórum: {e}', 'error')
        return redirect(url_for('student_dashboard'))
...
def forum_topico(topico_id):
    ...
    try:
        ...
        cur.execute('UPDATE forum_topicos SET visualizacoes = visualizacoes + 1 WHERE id = ?', (topico_id,))
        
        # Buscar tópico
        cur.execute('''
            SELECT t.*, u.username as autor_nome, u.user_type as autor_role,
                   a.titulo as aula_titulo
            FROM forum_topicos t
            JOIN users u ON t.autor_id = u.id
            JOIN aulas a ON t.aula_id = a.id
        ...
        # Buscar respostas
        cur.execute('''
            SELECT r.*, u.username as autor_nome, u.user_type as autor_role
            FROM forum_respostas r
            JOIN users u ON r.autor_id = u.id
            WHERE r.topico_id = ? AND r.ativo = 1
            ORDER BY r.melhor_resposta DESC, r.data_criacao ASC
        ''', (topico_id,))
        
        respostas = cur.fetchall()
        
        # Buscar tags do tópico
        cur.execute('''
            SELECT tag.nome, tag.cor
            FROM forum_topicos_tags tt
        ...
        return render_template('forum_topico.html', topico=topico, respostas=respostas, tags=tags)
        
    except Exception as e:
        flash(f'Erro ao carregar tópico: {e}', 'error')
        return redirect(url_for('forum'))
...
def forum_novo_topico():
    ...
    if request.method == 'POST':
        try:
            ...
            if not titulo or not conteudo or not aula_id:
                flash('Todos os campos são obrigatórios', 'error')
                return redirect(url_for('forum_novo_topico'))
            ...
            # Inserir tópico
            cur.execute('''
                INSERT INTO forum_topicos (titulo, conteudo, autor_id, aula_id, tipo)
                VALUES (?, ?, ?, ?, ?)
            ''', (titulo, conteudo, current_user.id, aula_id, tipo))
            ...
            for tag_id in tags:
                cur.execute('''
                    INSERT INTO forum_topicos_tags (topico_id, tag_id)
                    VALUES (?, ?)
                ''', (topico_id, tag_id))
            ...
            flash(f'Erro ao criar tópico: {e}', 'error')
            return redirect(url_for('forum_novo_topico'))
    ...
    try:
        ...
        return render_template('forum_novo_topico.html', aulas=aulas, tags=tags)
        
    except Exception as e:
        flash(f'Erro ao carregar formulário: {e}', 'error')
        return redirect(url_for('forum'))

@app.route('/forum/topico/<int:topico_id>/responder', methods=['POST'])
@login_required
def forum_responder(topico_id):
    """Responder a um tópico"""
    try:
        conteudo = request.form['conteudo'].strip()
        
        if not conteudo:
            flash('Conteúdo da resposta é obrigatório', 'error')
            return redirect(url_for('forum_topico', topico_id=topico_id))
        ...
        cur.execute('SELECT id FROM forum_topicos WHERE id = ? AND ativo = 1', (topico_id,))
        if not cur.fetchone():
            flash('Tópico não encontrado', 'error')
            return redirect(url_for('forum'))
        ...
        # Inserir resposta
            cur.execute('''
                INSERT INTO forum_respostas (topico_id, autor_id, conteudo)
                VALUES (?, ?, ?)
            ''', (topico_id, current_user.id, conteudo))
...
@app.route('/forum/resposta/<int:resposta_id>/votar/<tipo>')
@login_required
def forum_votar_resposta(resposta_id, tipo):
    ...
    try:
        ...
        cur.execute('''
            SELECT id FROM forum_votos 
            WHERE usuario_id = ? AND resposta_id = ?
        ''', (g.user['id'], resposta_id))
        ...
        if voto_existente:
            ...
            cur.execute('''
                UPDATE forum_votos SET tipo = ? WHERE id = ?
            ''', (tipo, voto_existente[0]))
        else:
            # Inserir novo voto
            cur.execute('''
                INSERT INTO forum_votos (usuario_id, resposta_id, tipo)
                VALUES (?, ?, ?)
            ''', (current_user.id, resposta_id, tipo))
        ...
        flash(f'Erro ao registrar voto: {e}', 'error')
    
    # Redirecionar de volta para o tópico
    cur = db.cursor()
    cur.execute('SELECT topico_id FROM forum_respostas WHERE id = ?', (resposta_id,))
    topico_id = cur.fetchone()[0]
    cur.close()
    
    return redirect(url_for('forum_topico', topico_id=topico_id))

@app.route('/forum/resposta/<int:resposta_id>/melhor')
@login_required
def forum_melhor_resposta(resposta_id):
    """Marcar resposta como melhor resposta (apenas autor do tópico)"""
    try:
        db = get_db()
        cur = db.cursor()
        ...
        cur.execute('''
            SELECT t.autor_id FROM forum_topicos t
            JOIN forum_respostas r ON t.id = r.topico_id
            WHERE r.id = ?
        ''', (resposta_id,))
        
        resultado = cur.fetchone()
        if not resultado or resultado[0] != current_user.id:
            flash('Apenas o autor do tópico pode marcar a melhor resposta', 'error')
            return redirect(url_for('forum'))
        
        # Desmarcar outras respostas como melhor
        cur.execute('''
            UPDATE forum_respostas SET melhor_resposta = 0
            WHERE topico_id = (SELECT topico_id FROM forum_respostas WHERE id = ?)
        ''', (resposta_id,))
        
        # Marcar esta resposta como melhor
        cur.execute('''
            UPDATE forum_respostas SET melhor_resposta = 1 WHERE id = ?
        ''', (resposta_id,))
        
        db.commit()
        cur.close()
        
        flash('Melhor resposta marcada com sucesso!', 'success')
        
    except Exception as e:
        flash(f'Erro ao marcar melhor resposta: {e}', 'error')
...
@app.route('/forum/topico/<int:topico_id>/fechar')
@login_required
def forum_fechar_topico(topico_id):
    """Fechar um tópico (apenas autor ou professor)"""
    try:
        db = get_db()
        cur = db.cursor()
        
        # Verificar se é o autor ou professor
        cur.execute('''
            SELECT t.autor_id, u.user_type FROM forum_topicos t
            JOIN users u ON t.autor_id = u.id
            WHERE t.id = ?
        ''', (topico_id,))
        ...
        autor_id, role = resultado
        
        if autor_id != current_user.id and role != 'professor':
            flash('Apenas o autor ou professores podem fechar tópicos', 'error')
            return redirect(url_for('forum'))
        
        # Fechar tópico
        cur.execute('''
            UPDATE forum_topicos SET status = 'fechado' WHERE id = ?
        ''', (topico_id,))
        
        db.commit()
        cur.close()
        
        flash('Tópico fechado com sucesso!', 'success')
        
    except Exception as e:
        flash(f'Erro ao fechar tópico: {e}', 'error')
    
    return redirect(url_for('forum_topico', topico_id=topico_id))

@app.route('/forum/topico/<int:topico_id>/excluir')
@login_required
def forum_excluir_topico(topico_id):
    """Excluir um tópico (apenas autor ou professor)"""
    try:
        db = get_db()
        cur = db.cursor()
        ...
        if autor_id != current_user.id and role != 'professor':
            flash('Apenas o autor ou professores podem excluir tópicos', 'error')
            return redirect(url_for('forum'))
        
        # Marcar como inativo (soft delete)
        cur.execute('''
            UPDATE forum_topicos SET ativo = 0 WHERE id = ?
        ''', (topico_id,))
        ...
        flash(f'Erro ao excluir tópico: {e}', 'error')
        return redirect(url_for('forum_topico', topico_id=topico_id))

@app.route('/forum/buscar')
@login_required
def forum_buscar():
    """Buscar tópicos no fórum"""
    query = request.args.get('q', '').strip()
    aula_id = request.args.get('aula_id', '')
    tag_id = request.args.get('tag_id', '')
    
    if not query and not aula_id and not tag_id:
        return redirect(url_for('forum'))
    ...
    try:
        ...
        sql = '''
            SELECT DISTINCT t.id, t.titulo, t.conteudo, t.tipo, t.status, t.data_criacao, t.visualizacoes,
                   u.username as autor_nome, u.user_type as autor_role,
                   a.titulo as aula_titulo,
                   COUNT(r.id) as num_respostas,
                   GROUP_CONCAT(tag.nome) as tags
        ...
        sql += ' GROUP BY t.id ORDER BY t.data_criacao DESC'
...
if __name__ == '__main__':
    ...
    host = '0.0.0.0'  # Importante para o Render
