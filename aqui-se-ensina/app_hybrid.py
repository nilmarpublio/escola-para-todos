...
try:
    ...
    pass  # Usar variáveis de ambiente padrão

# Importar modelos e autenticação
from models import User  # Usar modelos SQLite para desenvolvimento
...
def get_db():
    """Conectar ao banco de dados (SQLite local, PostgreSQL no Render)"""
    if 'db' not in g:
        # Verificar se estamos no Render (PostgreSQL) ou local (SQLite)
        database_url = os.getenv('DATABASE_URL')
        
        if database_url and os.getenv('FLASK_ENV') == 'production':
            ...
            try:
                ...
                # Fallback para SQLite se psycopg não estiver disponível
...
if __name__ == '__main__':
    ...
    print("🚀 Iniciando Aqui se Aprende (Modo Híbrido)")
